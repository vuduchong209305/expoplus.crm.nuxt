    ...
    <div class="flex flex-col h-full">
        ...
        <div class="flex-1 overflow-y-auto">
            ...
            <div v-else class="divide-y">
                <div v-for="item in availableCustomers" :key="item.id" class="flex items-center gap-3 px-2 py-3 hover:bg-gray-50">
                    <input type="checkbox" :value="item" :checked="isSelected(item.id)" @change="toggleSelect(item)" class="w-4 h-4 accent-indigo-500" />
                    <div class="flex-1">
                        ...
                        <div class="text-xs text-gray-400">
                            {{ item.company || 'Không có doanh nghiệp' }}
                        </div>
                    </div>
                </div>
                <div v-if="availableCustomers.length === 0" class="p-6 text-center text-gray-400"> Không có khách hàng khả dụng </div>
            </div>
        </div>
        <!-- PAGINATION -->
...
<script setup lang="ts">
	...
	const availableCustomers = computed(() => {
	    ...
	    return customers.value.filter(c => !existingIds.includes(c.id))
	})

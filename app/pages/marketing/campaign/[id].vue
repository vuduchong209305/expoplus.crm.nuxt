<template>
    ...
    <div class="p-4 lg:p-6">
        ...
        <form @submit.prevent="submit">
            ...
            <div class="bg-white px-5 py-4 rounded mb-5">
                <h4 class="font-semibold text-md mb-3">Thêm khách hàng mục tiêu</h4>
                ...
                <table class="w-full">
                    <thead class="border-b">
                        <tr>
                            ...
                            <th class="p-3 text-sm text-left font-medium" width="10%">Ngày khởi tạo</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(customer, index) in selectedCustomers" :key="index" class="border-b hover:bg-gray-100 transition-all">
                            ...
                            <td class="p-3">
                                <span class="text-gray-500 text-xs">{{ customer?.customer?.created_at }}</span>
                            </td>
        ...
        <Offcanvas :open="open" @close="open = false">
            <CustomerList v-if="open" :selected="selectedCustomers" @saved="handleSaved" @close="closeCanvas" :campaignId="campaignId"/>
        </Offcanvas>
    ...
    const submit = async () => {
        const res = await useNuxtApp().$apiFetch(`campaign/save`, {
            ...
            body: {
                ...
                customers: selectedCustomers.value.map(i => i.customer_id)
            }
        })
    ...
    const fetch = async() => {
        if (isEdit.value) {
            ...
            if (res.status) {
                ...
                note.value = campaign.note
                assigned_to.value = campaign.assigned_to

                startPicker.setDate(campaign.start_date)
                endPicker.setDate(campaign.end_date)
            ...
            if (result.isConfirmed) {
                ...
                if (isEdit.value) {
                    ...
                    selectedCustomers.value = selectedCustomers.value.filter(
                        item => !selectedIds.includes(item.customer_id)
                    )
